...
namespace Netcode.Transports.Facepunch
{
    ...
    public class FacepunchTransport : NetworkTransport, IConnectionManager, ISocketManager
    {
        ...
        [Tooltip("Virtual Steam relay port. The session publishes it in lobby data.")]
        [SerializeField, Min(0)] public int virtualPort;
        ...
        private LogLevel LogLevel => NetworkManager.Singleton != null
            ? NetworkManager.Singleton.LogLevel : Unity.Netcode.LogLevel.Normal;
        ...
        public override void Shutdown()
        {
            ...
            try
            {
                ...
                if (SteamClient.IsValid) client?.Close();
            }
            catch (Exception exception)
            {
                Debug.LogWarning($"[{nameof(FacepunchTransport)}] Connection cleanup: {exception.Message}");
            }
            finally
            {
                try
                {
                    if (SteamClient.IsValid) server?.Close();
                }
                catch (Exception exception)
                {
                    Debug.LogWarning($"[{nameof(FacepunchTransport)}] Socket cleanup: {exception.Message}");
                }
                finally
                {
                    // The coordinator owns Steam when it initialized the API.
                    // Switching NGO transports must not destroy its lobby API.
                    var ownsSteam = m_OwnsSteamClient;
                    m_OwnsSteamClient = false;
                    if (ownsSteam && SteamClient.IsValid) SteamClient.Shutdown();
                }
            }
        }
        ...
        public override bool StartClient()
        {
            ...
            return connectionManager != null || TryPrepareClient();
        }
        ...
        public override bool StartServer()
        {
            ...
            return socketManager != null || TryPrepareServer();
        }

        // Prepare before NetworkManager.Initialize so an invalid Steam socket
        // does not leave a partially initialized NGO session behind.
        public bool TryPrepareServer()
        {
            StartupError = null;
            if (socketManager != null) return true;
            try
            {
                if (!SteamClient.IsValid) throw new InvalidOperationException("Steam API is unavailable.");
                SteamNetworkingUtils.InitRelayNetworkAccess();
                for (var attempt = 0; attempt < 4; attempt++)
                {
                    ...
                    {
                        // A stale/parallel session can already own this virtual port.
                        virtualPort = 1 + (Guid.NewGuid().GetHashCode() & 0x7fff);
                    }
                }
            }
            catch (Exception exception)
        ...
        public bool TryPrepareClient()
        {
            ...
            {
                if (!SteamClient.IsValid) throw new InvalidOperationException("Steam API is unavailable.");
                if (targetSteamId == 0) throw new InvalidOperationException("Steam server ID is missing.");
                SteamNetworkingUtils.InitRelayNetworkAccess();
