        ...
        public void UpdatePhase1(float deltaTime)
        {
            ...
            if (InteractiveRigidbodyHandling)
            {
                ...
                if (_attachedRigidbody)
                {
                    ...
                    Vector3 angularDisplacement =
                        tmpAngularVelocityFromCurrentAttachedRigidbody * deltaTime;
                    Quaternion attachedBodyRotation = angularDisplacement.sqrMagnitude > 0.000001f
                        ? Quaternion.AngleAxis(
                            angularDisplacement.magnitude * Mathf.Rad2Deg,
                            angularDisplacement.normalized)
                        : Quaternion.identity;
                    Vector3 newForward = Vector3.ProjectOnPlane(
                        attachedBodyRotation * _characterForward, _characterUp).normalized;
        ...
        public void GetVelocityFromRigidbodyMovement(Rigidbody interactiveRigidbody, Vector3 atPoint, float deltaTime, out Vector3 linearVelocity, out Vector3 angularVelocity)
        {
            if (deltaTime > 0f)
            {
                ...
                if (angularVelocity != Vector3.zero)
                {
                    ...
                    Vector3 angularDisplacement = angularVelocity * deltaTime;
                    Quaternion rotationFromInteractiveRigidbody = angularDisplacement.sqrMagnitude > 0.000001f
                        ? Quaternion.AngleAxis(
                            angularDisplacement.magnitude * Mathf.Rad2Deg,
                            angularDisplacement.normalized)
                        : Quaternion.identity;
